...
function isValidProjectName(name) {
    ...
    if (CONFIG.RESERVED_NAMES.includes(name.toLowerCase())) {
...
class Spinner {
    constructor() {
        this.interval = null;
        this.frames = CONFIG.SPINNER_FRAMES;
        this.currentFrame = 0;
    }
    start(text) {
        if (!isInteractive()) {
...
function loadTemplates() {
    try {
        ...
        return [{
                ...
                description: 'Modern stack with Vite, React 19, TypeScript, and Tailwind CSS',
...
async function cloneTemplate(repoUrl, projectName) {
    const spinner = new Spinner();
...
async function selectTemplate(templates) {
    ...
    while (true) {
        const answer = await askQuestion(rl, `\x1b[33mEnter your choice (1-${templates.length}): \x1b[0m`);
